...
import scheduling.algorithms.PriorityBased;
import scheduling.assets.IntegerField;

public class PriorityBasedController implements Initializable{
    
    private PriorityBased pBased = new PriorityBased();
    ...
    void addProcess(ActionEvent event) {
        if(!priority.isEmpty() && !exexutionTime.isEmpty()){
            pBased.addProcess(new PriorityBased.Process(priority.getValue(), exexutionTime.getValue(), processColor.getValue()));
    ...
    void setCanvas() {
        ...
        for (int x = 0; x < width; x+=30, scale += 5) {
